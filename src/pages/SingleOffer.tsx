import { Avatar, Button, Carousel, Divider, Image, Layout, List, Space, Spin, Tag, Typography } from 'antd'
import moment from 'moment'
import { useEffect, useState } from 'react'
import { BiCalendar, BiCheck, BiCheckCircle, BiExpandAlt, BiTimeFive } from 'react-icons/bi'
...
const SingleOffer = () => {
	...
	return (
		<Layout>
			...
			<Layout.Content style={{ display: 'flex', flexDirection: 'column', padding: matchesXS ? '20px' : '50px' }}>
				<Space direction='horizontal' style={{ justifyContent: 'space-between' }} wrap>
					<Space direction='vertical'>
						...
						<Space>
							<Space align='start'>
								...
								<Typography.Text type='secondary'>{transformNumber(offer.area!)} m²</Typography.Text>
							</Space>
					...
					{offer.user._id !== user!.id && (
						<Space direction='vertical' style={{ paddingTop: matchesXS ? '20px' : '0px' }}>
							<Button type='primary' size='large' onClick={() => onGoToCheckout('purchase')}>
								Purchase
							</Button>
							{offer.is_for_rent && (
								<>
									...
									<Button block type='link' size='large' onClick={() => onGoToCheckout('rent')}>
										Rent
									</Button>
								</>
							)}
				...
				>
					<List.Item style={{ justifyContent: 'flex-start' }}>
						...
						<Typography.Text type='secondary'>{offer.property_type}</Typography.Text>
					</List.Item>
					<List.Item style={{ justifyContent: 'flex-start' }}>
						<Typography.Text strong style={{ width: '150px' }}>
							Construction year:
						</Typography.Text>
						<Typography.Text type='secondary'>{offer.construction_year}</Typography.Text>
					</List.Item>
					{offer.is_parking && (
						<List.Item style={{ justifyContent: 'flex-start' }}>
							<Typography.Text strong style={{ width: '150px' }}>
				...
				>
					...
					<List.Item style={{ justifyContent: 'flex-start' }}>
						...
						<Typography.Text type='secondary'>{offer.surroundings}</Typography.Text>
					</List.Item>
					<List.Item style={{ justifyContent: 'flex-start' }}>
						<Typography.Text strong style={{ width: '150px' }}>
							Nearby:
				...
				<Space direction='vertical' style={{ marginTop: '50px' }}>
					...
					>
						...
						<Space direction='vertical' style={{ rowGap: 0 }}>
							...
							<Typography.Text type='secondary'>{offer.user.email}</Typography.Text>
						</Space>
					</Space>
					<Button size='large' type='default' onClick={() => navigate(`/users/${offer.user._id}`)}>
						More information
					</Button>
				</Space>
			</Layout.Content>
