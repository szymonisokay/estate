...
const SingleOffer: React.FC<ComponentType> = ({ offer, type }) => {
	...
	const cardActions = [
		...
		/>,
		<BiNavigation size={20} key='navigate' onClick={() => onClick('navigate')} />,
	]
	...
	const removeBookmark = useCallback(async () => {
		const { msg } = await BookmarksService.removeBookmark(offer._id!, getToken())
		toast.info(msg)
	}, [offer._id, getToken])
	...
	return (
		...
		>
			<Card.Meta
				title={
					<Space direction='vertical' align='start' style={{ width: '100%' }} className='offercard_meta'>
						<Space
						...
						<Link to={`/offers/${offer._id}`} style={{ whiteSpace: 'pre-wrap' }}>
